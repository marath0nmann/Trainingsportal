...
## v35
- Fix: Typ-Farben aus den Einstellungen werden jetzt tatsächlich angewendet – `applyTypenFarben()` in `applyConfig()` injiziert dynamisch ein `<style>`-Tag mit `.block-typ-{slug}` Regeln (border-top bei Karten, border-left + Textfarbe bei Gruppen-Titeln, border-left bei Kalender-Einheiten)
- CSS: `.bloecke-gruppe-titel` neu strukturiert mit Typ-Badge (`.bloecke-gruppe-typ`) und Anzahl-Chip (`.bloecke-gruppe-count`); Typ-Badge zeigt farbigen linken Balken
