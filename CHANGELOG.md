...
- **Die Standard-Uhrzeit je Wochentag gilt jetzt überall.** Sie lag nur im Block-Formular; im Editor blieb das Uhrzeitfeld bei einer neuen Einheit leer. Beim Ändern des Datums wird sie nachgezogen – aber nur, solange die Zeit leer ist oder noch dem Standard des vorherigen Tages entspricht. Eine von Hand gesetzte Zeit bleibt stehen.
- Drei identische `val()`-Kopien (Editor, Blöcke, Termin-Modal) sind durch `FELDER.wert()` ersetzt.
...
- **Die Karte ist zugeklappt, bis jemand sie braucht.** 380 Pixel und ein Leaflet-Ladevorgang bei jedem Seitenaufruf sind für die meisten Besuche zu viel. Ein Klick auf „🗺️ Karte der Wettkämpfe" klappt sie auf; zugeklappt wird Leaflet gar nicht erst geladen. Auch diese Wahl bleibt erhalten.
- **Kalender oder Liste ist jetzt eine sichtbare, dauerhafte Entscheidung.** Unter 720px leitete `#kalender` bisher *jedes Mal still* auf die Liste um – außer eine Sitzungsmarke sagte etwas anderes, die beim nächsten Tab-Wechsel wieder verfiel. Jetzt entscheidet der sichtbare Umschalter, die Wahl hält dauerhaft (`localStorage`), und die Bildschirmbreite bestimmt nur noch die Voreinstellung beim allerersten Besuch.
- **Die Kalenderfarbe einer Gruppe gehört zu den Gruppen-Stammdaten.** Der Farbwähler steckte in der Reiterleiste der Trainingsplanung – eine global für alle Athleten wirkende Einstellung an einer Navigationsleiste, zurückzusetzen nur per Rechtsklick und auf dem Handy überhaupt nicht erreichbar. Er steht jetzt in **Admin → Gruppen** neben dem Gruppennamen, mit einem sichtbaren ↺ zum Zurücksetzen. Die Reiter zeigen die Farbe weiterhin als Punkt an.
- **Neue Kachel „Planungsstand" auf der Übersicht – nur für Trainer und Admins.** Beantwortet die Trainerfrage „was fehlt": Wochen ohne Training in den nächsten vier Wochen und Termine ohne Treffpunkt, jeweils mit Direktlink dorthin.

## v340
- **Die gemeinsame Filterleiste steht jetzt über jeder Liste.** Bisher nutzten sie nur drei von acht Listen – wer sie in Admin → Trainings kennengelernt hatte, fand sie bei Trainingsblöcken, Strecken, Treffpunkten, Gruppen und Athletenplänen nicht wieder. Alle fünf haben sie jetzt, mit Filterspalten, die zur jeweiligen Liste passen: Trainingstyp, Sichtbarkeit, Trainingsgruppe, Strecke und Segmente bei den Blöcken; Verwendung, Herkunft und Längenklasse bei den Strecken; Koordinaten und Kartenlink bei den Treffpunkten; Zugriffsstufe bei den Athletenplänen; Mitglieder bei den Gruppen.
- **Paginierung in Admin → Trainings.** Die Tabelle zeichnete bis zu 2000 Zeilen auf einmal. Jetzt 50 pro Seite mit der Zeile „Seite 2 von 14 · 693 Einträge" darunter – die zugleich beantwortet, wie viel überhaupt da ist. Das Kopf-Kästchen wählt die sichtbare Seite, nicht alle 693; die Auswahl bleibt beim Blättern bestehen. Geladen wird weiterhin die volle Menge, weil die Filterleiste ihre Trefferzahlen clientseitig über alle Zeilen zählt.
- Athletenliste, Gruppen und Papierkorb nutzen `.data-table` aus der geteilten `app.css` und damit deren mobile Regeln. Wettkampfplanung und Admin → Trainings behalten bewusst `.karten-tabelle`: sie klappen unter 720px in Karten auf, was `.data-table` mit seinem horizontalen Scrollen wieder kaputt machen würde.
- **Fix: Treffpunkte löschten nach dem Speichern die Admin-Reiter.** Die Seite zeichnete sich nach Anlegen, Bearbeiten oder Löschen in `#main-content` neu – sie hängt aber unter Admin → Treffpunkte in `#admin-content`. Das Untermenü verschwand dabei mit. Der Container wird jetzt gemerkt.

## v339
- **Keine nativen Browser-Dialoge mehr.** 34 Stellen öffneten `confirm()`, `alert()` oder `prompt()` – Fenster, die auf jedem Gerät anders aussehen, weder Vereinsfarbe noch Kontext tragen und sich auf dem Handy leicht wegtippen lassen. Die 20 Rückfragen laufen jetzt über `confirmModal()`, die 9 Meldungen sind Toasts geworden, und die 5 Eingaben nutzen das neue `promptModal()`. Alle drei stammen aus der geteilten `09a_utils_shared.js` – dieselben Dialoge wie im Statistikportal.
- **Rückfragen aus einem offenen Formular heraus löschen es nicht mehr.** `confirmModal()` hing bisher an `showModal()` und ersetzte damit den Modal-Inhalt; nach „Abbrechen" wäre der Editor weg gewesen. Die Dialoge legen sich jetzt als eigenes Overlay darüber (Statistikportal v1563).
- **Acht Toast-Kopien wurden eine.** `benachrichtigen()` bzw. `notify()` war in `02_app.js`, `04_editor.js`, `07_settings.js`, `08_bloecke.js`, `10_planung.js`, `12_admin_trainings.js`, `13_meinplan.js` und `16_admin_wettkampf.js` jeweils eigenständig definiert – mit unterschiedlicher Standzeit. Jetzt nutzen alle 153 Aufrufe die geteilte Funktion. Die gestapelte Anordnung und die volle Breite auf dem Smartphone bleiben erhalten: `addons.css` hebt für beide Klassen die feste Positionierung aus `app.css` auf. Der Undo-Toast beim Löschen privater Einheiten bleibt als eigener Baustein bestehen.
...
- **Neu: Admin → Papierkorb.** Die API löscht fachlich nie hart – `archiviereUndLoesche()` legt jede Zeile vollständig als JSON in `training_geloescht` ab –, es gab bisher nur keinen Weg zurück. Die Seite zeigt Art, Eintrag, Zeitpunkt, Urheber und Grund, filtert über die gemeinsame Filterleiste und stellt einzelne Datensätze wieder her. Zusammengehörige Teile kommen mit: die Segmente einer Einheit ebenso wie Segmente und Gruppen eines Blocks. Reine Kindtabellen erscheinen nicht als eigene Zeilen.
- **Neu: Admin → Wartung.** Die einmaligen Werkzeuge „Migration: Einheiten → Trainingsblöcke" und „Segmente aus Titeln parsen" standen mitten in den täglichen Einstellungen, zwischen Standard-Uhrzeiten und Trainingstypen. Sie haben jetzt einen eigenen Reiter, wie im Statistikportal.
- **Admin-Untermenü mit Icons und neuer Reihenfolge:** System · Gruppen · Trainings · Wettkämpfe · Treffpunkte · Strecken · Einstellungen · Wartung · Papierkorb – Stammdaten vorn, Werkzeuge hinten, wie im Statistikportal. „Trainings" und „Papierkorb" tragen dieselben Badges wie der Hauptmenüpunkt.
